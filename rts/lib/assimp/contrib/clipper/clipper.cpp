...
float Area(const OutRec &outRec, bool UseFullInt64Range)
{
  OutPt *op = outRec.pts;
  if (UseFullInt64Range) {
    Int128 a(0);
...
{
  return (pt1.Y == pt2.Y) ?
    HORIZONTAL : (float)(pt2.X - pt1.X) / (float)(pt2.Y - pt1.Y);
}
//---------------------------------------------------------------------------
...
void SetDx(TEdge &e)
{
  ...
  else e.dx = (float)(e.xtop - e.xbot) / (float)(e.ytop - e.ybot);
}
//---------------------------------------------------------------------------
...
bool FirstIsBottomPt(const OutPt* btmPt1, const OutPt* btmPt2)
{
  ...
  float dx1p = math::fabs(GetDx(btmPt1->pt, p->pt));
  p = btmPt1->next;
  while (PointsEqual(p->pt, btmPt1->pt) && (p != btmPt1)) p = p->next;
  float dx1n = math::fabs(GetDx(btmPt1->pt, p->pt));

  p = btmPt2->prev;
  while (PointsEqual(p->pt, btmPt2->pt) && (p != btmPt2)) p = p->prev;
  float dx2p = math::fabs(GetDx(btmPt2->pt, p->pt));
  p = btmPt2->next;
  while (PointsEqual(p->pt, btmPt2->pt) && (p != btmPt2)) p = p->next;
  float dx2n = math::fabs(GetDx(btmPt2->pt, p->pt));
...
{
  long64 steps = std::max(6, int(std::sqrt(math::fabs(r)) * math::fabs(a2 - a1)));
...
DoublePoint GetUnitNormal( const IntPoint &pt1, const IntPoint &pt2)
{
  ...
  float f = 1 *1.0/ std::sqrt( dx*dx + dy*dy );
  dx *= f;
  dy *= f;
